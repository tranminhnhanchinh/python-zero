...
I.e. all integers from start to end inclusive of start and end are also present and also we are given an integer k
We have to return the Powerfull Integer. A powerful Integer is an integer that occurs at least k times. 
...
for item in intervals:
    for i in range(list(item)[0], list(item)[1]+1):
        breakList.append(i)
uniqueList = list(set(breakList))
dictCount = {}
for each in uniqueList:
    dictCount.update({each: breakList.count(each)})

#Perfect Number is number that occurs k times
listPerfect = []
for num, times_occur in dictCount.items():
    if times_occur == k:
        listPerfect.append(num)

perfectNumber = max(listPerfect)
times_occur = dictCount.get(perfectNumber)
print(f"The Perfect Number is: {perfectNumber} with {times_occur} times occurs")